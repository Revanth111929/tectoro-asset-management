...
class InventoryValidator:
    ...
    @staticmethod
    def validate_asset_available(asset: Asset) -> Tuple[bool, Optional[str]]:
        """
    ...
    @staticmethod
    def validate_multiple_assets_per_employee(emp_id: str, new_asset: Asset) -> Tuple[bool, Optional[str]]:
        """
        ...
        for asset in assigned_assets:
            if asset.id == new_asset.id:
                return False, f"This asset is already assigned to Employee '{emp_id}'"
            
            # Check if same serial number (same physical device)
            if asset.serial_number == new_asset.serial_number:
                return False, f"Asset with serial number '{new_asset.serial_number}' is already assigned to Employee '{emp_id}'"
        
        # Different assets allowed - return True
        return True, None
    ...
    def validate_asset_assignment(asset_id: int, emp_id: str) -> Dict:
        ...
        # 6. Validate multiple assets (informational)
        is_valid, error = InventoryValidator.validate_multiple_assets_per_employee(emp_id, asset)
        if not is_valid:
            result['valid'] = False
            result['errors'].append(error)
        else:
            # Add informational message if employee has other assets
            assigned_assets = InventoryValidator.get_employee_assigned_assets(emp_id)
            if assigned_assets:
                asset_list = ', '.join([f"{a.category} ({a.asset_name})" for a in assigned_assets])
                result['warnings'].append(f"Employee already has {len(assigned_assets)} asset(s): {asset_list}")
